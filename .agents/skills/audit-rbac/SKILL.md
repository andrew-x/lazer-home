---
name: audit-rbac
description: Use when you want to audit RBAC/permissions end-to-end — verifying the role matrix, every action's gate, and that nothing bypasses access control. Read-only; flags vulnerabilities.
---
...
If the user named a scope when invoking this skill (a domain or path, e.g. "staff" or
"src/actions/crm"), audit only that scope; otherwise audit the whole codebase.
...
`.claude/rules/permissions.md` (mirrored for Codex in the nested `src/**/AGENTS.md`
files and, for permissions, in the root `AGENTS.md`). Read all three first.
