...
/**
 * Toggle a task's completion. Gated on `crm.edit`. `completedAt` is stamped when
 * `done` flips true and cleared back to null when the task is reopened, so the
 * completion date always tracks the flag. `.returning()` the parent FKs guards
 * the row and drives revalidation.
 */
export const setTaskDone = secureActionClient
  .metadata({ action: "set-task-done", permission: { crm: ["edit"] } })
