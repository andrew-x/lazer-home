 ...
 * self-join alias of it — and select the result.
 *
 * Left-joined aliases can be absent, so the value is nullable at the type level:
 * call as `contactNameSql<string | null>(someAlias)` there; the default `string`
 * suits inner-joined contacts.
 *
 * Not usable for a projection that also carries a **left join**: a raw `sql`
 * expression isn't attributable to a table, so Drizzle widens it to nullable
 * regardless of this generic. Project `firstName`/`lastName` and compose with
 * `contactName` from `@/lib/crm/contact-name` instead — that's what
 * `getContactDetail`'s relationship query does.
 */
