 ...
 * `/people/levels`, `/people/compensation-plans`, `/people/bonus-payments` and
 * `/people/profile-completeness`. The sidebar's parent nav entry points here, so
 * send each viewer to the first surface they may use.
 *
 * All four gates happen to resolve to {manager, admin} today, so in practice this
...
export default async function PeopleManagementPage() {
  ...
  if (userHasPermission(user, PROFILE_COMPLETENESS_ACCESS)) {
    redirect("/people/profile-completeness");
  }
  notFound();
}
