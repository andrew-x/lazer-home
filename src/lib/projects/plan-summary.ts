/**
 * Pure helpers for the project-plan summary tiles — the date-range labels shown
 * above the planner grid. A client-importable module (no `db`/drizzle, no React) so
 * the opportunity's Project-plan tab and the standalone project detail page render
 * identical stats from one source.
 *
 * {@link rangeOf} is also the window helper behind
 * `@/lib/projects/delivery-coverage`, so a coverage gap is measured against exactly
 * the span the Dates tile prints.
 */
