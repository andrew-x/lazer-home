...
- **`isActive`** = no `terminationDate`. **`isBillable`** = role NOT in {LEADERSHIP, SALES, SOLUTIONS, OPERATIONS}. **`utilizationTarget`** = `isBillable ? 100 : 0`.
