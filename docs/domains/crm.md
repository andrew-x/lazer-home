...
- **Server layer** — `src/actions/crm/`:
  ...
  - `searchCompanies.ts` / `searchContacts.ts` / `searchStaff.ts` — type-ahead pickers, `secureActionClient` gated `crm.edit` (they back CRM entry, itself a write, so the gate stops them being used to enumerate the roster past the page gate). Each returns up to `SEARCH_LIMIT` `{ id, name }` by `ilike` match; **a blank query returns nothing** (search only runs once the user types). Two share the query surface via `src/lib/search.ts`, which exports `SEARCH_LIMIT` (10) and `searchQuerySchema` (a single `query` string **trimmed on parse**, so whitespace collapses to the blank-query short-circuit); the per-action query bodies differ (different tables/columns). Nuances: `searchStaff` filters `isActive = true` (**only active staff**), and `searchContacts` matches **first name / last name / email** (not just name). All three also share the LIKE-metacharacter escaper `escapeLike` in `src/lib/like.ts` (extracted from `searchCompanies`).
