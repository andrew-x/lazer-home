...
Ship a read-only report at **`/reporting/utilization`**, computed by a pure module
...
- **The Dashboards nav parent is no longer gated** (that section is called **Reporting** as of
  2026-08-03 — [ADR 0055](./0055-nav-dashboards-vs-people-management.md)), and `/reporting` no longer `notFound()`s.
