...
| [0055](./0055-self-evaluations-dated-records-with-snapshotted-answers.md) | Staff self-evaluations: a **new dated table** (not the generic `responses` table — a periodic record needs N rows per *occasion*), each answer storing its own **section/prompt snapshot** so a record renders entirely from itself; `selfRating` a **typed column** on a **separate pg type** built from peer feedback's shared tuple; **reads own-always ⁄ `ratings.view`, writes author-only with no admin override**; edits refused once the question set moves on. **[ADR 0032](./0032-staff-rating-levels-effective-dated-manager-only.md) is not weakened** — never join `staff_rating` beside a self-rating | accepted; no matrix change |
