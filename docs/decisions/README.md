...
| [0052](./0052-project-budgets-and-margin.md) | Project budgets & margin: billing model + fee on `projects`, **one company-wide rate card in code** (no per-project rates), hours from real weekdays (not grid buckets), fixed-fee margin only at project level, new `projects.viewMargin` gate with masking in the read | accepted; **§1–2 self-reversed same-day** — a `project_role_rates` table was built and migrated, then dropped (`0014` creates, `0015` drops) in favour of the code-owned card |
