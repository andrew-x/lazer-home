...
| [domains/projects.md](./domains/projects.md) | Projects (client engagements) + delivery managers + role/staffing lines (six disciplines) + **budgets, the code-owned rate card (line of business × discipline) with a rate *snapshotted* onto each role, and plan margin** (cost/margin gated on `projects.viewMargin`; rates and the fixed-fee hourly-value comparator are not) + the list's **derived risk tags** (code-owned thresholds, judged in CAD) + **delivery notes** (dated write-ups with a 1–10 health rating; list health derived from the latest note, ungated) + the `/projects` list itself: a **sortable, server-ordered table under status tabs** (nulls last both ways; margin *ordering* gated like the figures) |
