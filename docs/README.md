...
| [domains/performance.md](./domains/performance.md) | Peer feedback + the two dashboards (`/performance/compensation` and `/performance/levels`, separately gated; `/performance` is a redirect) + staff rating levels L0–L4 with per-role subratings (built); reviews, goals, growth (proposed) |
