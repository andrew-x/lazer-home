...
| [domains/utilization.md](./domains/utilization.md) | The **Utilization report** (`/analytics/utilization`, built) — plan vs. actuals in **two never-summed series**; open page, but cross-person **confirmed hours gated on `timesheets.edit`**; plus the table distinguishing the **three surfaces** that say "utilization" over three different windows |
