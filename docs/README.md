...
| [domains/performance.md](./domains/performance.md) | Peer feedback (incl. the per-person profile tab) + the three **Dashboards** (`/dashboards/compensation`, `/dashboards/bonuses`, `/dashboards/levels` — separately gated, aggregate & anonymized) + the **People management** write screens (`/people/levels`, `/people/compensation-plans`, `/people/bonus-payments`; both section indexes are redirects) + staff rating levels L0–L4 with per-role subratings + compensation change plans + **performance review notes** + **bonus payments** (`staff_bonus_payment` — dated payments, date-of-payment attribution, the type × LoB/role matrix) (built); review cycles, goals, growth (proposed) |
