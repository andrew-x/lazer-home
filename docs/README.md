...
| [domains/allocations.md](./domains/allocations.md) | Staffing people onto projects over time; the `/allocations` planner view (built) + its manager-only inline notes + the per-cell **remaining-capacity meter** (cross-project load, PTO netted out, over-allocation flagged — but **not** a capacity model) |
| [domains/timesheets.md](./domains/timesheets.md) | Weekly time capture (built); approval + billing deferred |
| [domains/utilization.md](./domains/utilization.md) | The **Utilization report** (`/dashboards/utilization`, built) — plan vs. actuals as two never-summed series behind a **`Planned \| Logged` basis toggle** (the off-screen series drives **deviation flags**); confirmed roles only; open page, but the whole **Logged basis is gated cohort-wide on `timesheets.edit`** |
