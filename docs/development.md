...
- **Slack channel links (optional):** `SLACK_BOT_TOKEN` (must start with `xoxb-` — `env.ts` refines it, so a *user* token fails at boot) turns the integration on; `SLACK_TEAM_ID` optionally scopes the deep links to one workspace. **Leaving both unset is a fine local default** — the channel controls simply hide themselves, which is the state most local work wants. Scopes and the Slack-app setup: [guides/slack.md](./guides/slack.md).
- **Google Drive folders (optional):** set **all three** of `GOOGLE_DRIVE_ROOT_ID`, `NEXT_PUBLIC_GOOGLE_PICKER_API_KEY` and `NEXT_PUBLIC_GOOGLE_PICKER_APP_ID` — `isDriveConfigured()` requires the lot, since browsing needs only the drive id but the Picker needs its own two. **Leaving them unset is a fine local default.** If you do turn it on locally, point `GOOGLE_DRIVE_ROOT_ID` at a **separate scratch shared drive** (there's no `test-` name prefix, unlike Slack), and note you'll have to **sign out and back in once** — the Drive OAuth scope grants nothing to an existing session. Full runbook: [guides/google-drive.md](./guides/google-drive.md).
