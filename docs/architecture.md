...
src/
  ...
  lib/
    ...
    project-derived.ts       deriveProjectStatus / deriveProjectLinesOfBusiness (pure, +.test.ts) — a project stores no status/LoB; both are computed from its roles, shared by reads + UI + tests (replaced the deleted project-status.ts; [ADR 0033](./decisions/0033-line-of-business-on-role-derived-project-status.md))
    ...
    staff-rating.ts          RATING_LEVELS + MIN/MAX_RATING_LEVEL + formatLevel/formatAverageLevel + encode/decodeLevelValue (pure, client-safe) — single source for the staff_rating CHECK bounds, the eval zod schema, and level display (mirrors feedback-rating.ts; [ADR 0032](./decisions/0032-staff-rating-levels-effective-dated-manager-only.md))
    ...
    timesheet-status.ts      TIMESHEET_STATUSES tuple + type + labels + DEFAULT (pure, client-safe; mirrors line-of-business.ts per ADR 0016) — single source; the timesheet_status pgEnum in timesheets-schema.ts is BUILT FROM it (values unchanged, no migration)
