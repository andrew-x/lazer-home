...
src/
  ...
  actions/<domain>/          the single entry point for ALL DB access (ADR 0010)
    ...
    crm/                     get{Companies,Contacts}Page + getOpportunitiesBoard (the kanban read; there's no getOpportunitiesPage) + get{Company,Contact}Detail + getOpportunity (server-only reads) + loadOpportunityDetail (client-triggered 'use server' read — load* not get*, since get* is reserved for server-only reads) + search{Companies,Contacts,Staff} (type-ahead) + {create,update}{Company,Contact} + update{Company,Contact}Field (+ .schema.ts — the inline detail-page field edits: a discriminated union on `field` (owner/location, plus relationshipStrength for contact) mirroring updateOpportunityField, so only the changed column is written; crm.edit-gated — these REPLACED the five deleted single-column actions update{Company,Contact}Owner / update{Company,Contact}Location / updateContactRelationshipStrength) + createOpportunity + opportunityHasProject (server-only guard backing the one-project-per-opportunity rule) + updateOpportunityField (the drawer's write path: field-scoped edits, a discriminated union on `field`, so only the changed slice is written — no stale-write clobber) + updateOpportunityPosition — all writes gated crm.edit (+.schema.ts). (The old full-record updateOpportunity + .schema.ts have been DELETED — updateOpportunityField is the sole drawer write path; writeOpportunityLinks stays live via createOpportunity.) Shared non-action helpers: revalidate.ts (revalidateCompany/revalidateContact — list+detail path, mirroring staff's revalidateStaffProfile) + entryMutations.ts (addEntry/updateEntryBody/deleteEntry core the nine entry-CRUD wrappers delegate to; reads via selectEntries in entryViews.ts) + opportunityLinks.ts (replaceOpportunity{Contacts,Owners,SourceStaff,SourceContacts} + writeOpportunityLinks — the four people-junctions) + assertOpportunityTransitionAllowed.ts (the single ADR 0024 delivery-transition enforcement, shared by updateOpportunityPosition + updateOpportunityField) + contactChecks.ts (assertValidManager/mapContactEmailConflict, shared by create+update contact). Source/status enum tuples + labels live in src/lib/crm/opportunity.ts, imported directly by the schema + pgEnum + UI (back-compat re-export shims removed)
    projects/                getProjectsList (server-only open read backing /projects: getProjectsInBuckets [full status sections] + getProjectsPage [offset/limit page] + getDeliveryManagerOptions [the dm-filter option set]; derives status/LoB + a role date range, filters by project/company name + line of business + delivery-manager id; REPLACED the deleted getProjectsPage.ts/ProjectRow) + searchStaff/searchCompanies (type-ahead, projects.edit-gated) + createProject — writes gated projects.edit (+.schema.ts, pure). Derived-status bucketing done in SQL by lib/projects/project-status-sql.ts (derivedStatusCondition over four buckets tentative/paused/active/other[cancelled-only] — correlated EXISTS, LOCKSTEP with the JS mirror statusesMatchBucket + guarded by project-derived.test.ts)
  ...
  components/                React components; ui/ = vendored shadcn primitives,
                             ...
                             projects/ = add-project-dialog (minimal: name + company) + project-card + projects-grid (ProjectsGrid + ProjectsSection) + projects-list-filters (URL-backed search + LoB + delivery-manager filters) + project-status-badge + detail/ (project-detail-view) + opportunity-plan/ (the weekly Gantt-like opportunity planner — opportunity-project-plan entry + planner-grid + edit-project-dialog + role-dialog + extend-dialog; mounted inside the CRM opportunity detail sheet),
  ...
  lib/                       organized into domain subfolders (below); every file kept its name — only the import prefix changed (@/lib/<file> → @/lib/<group>/<file>). db/ was NOT moved. ([ADR 0036](./decisions/0036-lib-organized-by-domain-subfolders.md))
    core/                    cross-cutting plumbing:
      ...
      list-href.ts           shared querystring helpers for the URL-backed CRM list views — the filter bars (companies/contacts/opportunities) + PaginationControls all rebuild the current querystring differing only by base path + page-key, so hrefWith/buildHref lives here once
    auth/                    auth + access control:
      auth.ts auth-client.ts better-auth server + client (admin plugin wired with ac/roles); getCurrentUser/checkAuth
      permissions.ts         RBAC single source of truth: statement, roles, matrix, userHasPermission/requirePermission (permissions.test.ts asserts the matrix — ADR 0037 removed the broad unit suite but kept this RBAC-matrix test; two small pure-logic tests have since been added — allocations-grid.test.ts + timesheet-week.test.ts — so `bun run check` now runs three *.test.ts files; [ADR 0037](./decisions/0037-unit-tests-removed-except-rbac-matrix.md))
    ...
    format/                  display + money:
      ...
      fx.ts                  AED_PER_USD + FALLBACK_USD_RATES + convert(amount, from, to, usdRates) — pure USD-cross-rate FX conversion, client-importable; consumed by /performance (rates from getExchangeRates) ([ADR 0029](./decisions/0029-external-fx-rates-and-currency-normalization.md))
    crm/                     CRM enum tuples + derived helpers:
      ...
      (also opportunity.ts, company-status.ts, relationship-strength.ts)
    ...
    projects/                project enum tuples + derived status/LoB:
      project-role-type.ts   PROJECT_ROLE_TYPE tuple + labels (pure, client-importable) — single source for the projectRoleTypeEnum pgEnum, the project-role zod schema, and forms (ENGINEER/DESIGNER/ARCHITECT/QA/SPECIALIST)
    ...
    performance/             performance aggregation + feedback enum:
      ...
      performance-stats.ts   computeGroupStats + computeByRole (pure aggregation over normalized comp rows; empty group → null, not NaN) — the /performance dashboard's math (comp/rate-per-level table reuses computeByRole)
