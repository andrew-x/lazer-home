...
src/
  ...
  app/
    ...
    (app)/                   AUTHENTICATED route group — layout.tsx gates via getCurrentUser() + getCurrentStaff()→redirect, renders AppShell
      ...
      performance/page.tsx  (comp & headcount analytics — server page, notFound()s non-viewCompensation users)
  ...
  actions/<domain>/          the single entry point for ALL DB access (ADR 0010)
    ...
    crm/                     get{Companies,Contacts,Opportunities}Page + get{Company,Contact}Detail (server-only reads) + loadOpportunityDetail (client-triggered 'use server' read — load* not get*, since get* is reserved for server-only reads) + search{Companies,Contacts,Staff} (type-ahead) + {create,update}{Company,Contact} + {create,update}Opportunity + updateOpportunityPosition — all writes gated crm.edit (+.schema.ts). Shared non-action helpers: opportunityLinks.ts (writeOpportunityLinks — the four people-junctions, used by create+update) + contactChecks.ts (assertValidManager/mapContactEmailConflict, shared by create+update contact). Source/status enum tuples + labels live in src/lib/opportunity.ts, imported directly by the schema + pgEnum + UI (back-compat re-export shims removed)
  ...
  components/                React components; ui/ = vendored shadcn primitives,
                             ...
                             performance/ = performance-dashboard (client: filters, CAD/USD toggle, KPI cards, by-role table, distribution scatter) + stat-card (reusable KPI tile, extracted from the Home page) + compensation-scatter (hand-rolled inline-SVG single-series scatter, no charting lib),
                             ...
                             plus top-level shared cells empty-cell.tsx + external-link.tsx + pagination-controls.tsx (Link-based prev/next, reused by CRM + projects tables)
  hooks/                     useDebouncedValue (debounce, used by company-combobox), useMobile
  lib/
    ...
    performance-stats.ts     computeGroupStats + computeByRole (pure aggregation over normalized comp rows; empty group → null, not NaN) (+.test.ts) — the /performance dashboard's math
    ...
    db/
      ...
      schema.ts              barrel: re-exports auth-schema + staff-schema + crm-schema + opportunities-schema + projects-schema + performance-schema + timesheets-schema (one import for the whole schema)
