...
`getStaffProfile` returns the profile fields plus the **latest** `staffEmployment` row via the [ADR 0007](./decisions/0007-staff-employment-effective-dating.md) ordering. Enums display via `humanizeEnum()`, dates via the timezone-safe `formatDate()`, and avatar fallbacks via `initialsFor()` — all from `src/lib/format.ts` (`initialsFor` was moved here to be shared; see the dates gotcha in [staff-profiles.md](./domains/staff-profiles.md)).

**Edit (Links + Client intro cards).** Each editable card puts its **Edit button in the card header via `CardAction`** (`src/components/ui/card.tsx` — a header slot that right-aligns into the grid; use it for header-level affordances rather than absolutely positioning a button). The buttons live in `edit-links-dialog.tsx` / `edit-client-intro-dialog.tsx` — client components taking a **`staffId` prop** and opening a `Dialog`. The **inner form is only mounted while the dialog is `open`** (`{open ? <Form …/> : null}`), so its `defaultValues` always reflect the latest server data after a save rather than going stale. The form uses **`useHookFormAction`** (tight binding; see `.claude/rules/forms.md`) wired to the `updateStaff*` action with the imported `.schema.ts` as the `zodResolver`; the Save button drives `loading` off `action.isPending`, server errors read off `action.result.serverError`, `onSuccess` closes the dialog. The edit is currently shown to **any signed-in user** on anyone's profile (ADR 0012) — gate this when RBAC lands.
