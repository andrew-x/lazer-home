...
- **`add-opportunity-dialog.tsx`** is the form (a large dialog, `sm:max-w-2xl`). Company is a single required `CompanyComboboxField` (the same shared picker the contact form uses, here passed an `error`); related **contacts**, **owners**, referral **source contacts**, and referral **source staff** are multi-selects (`entity-multi-combobox.tsx`). The conditional referral rules (`staff_referral` needs ≥1 source staff, `contact_referral` needs ≥1 source contact) are validated by the shared `createOpportunity.schema.ts` via the form resolver, matching the server `superRefine`.
