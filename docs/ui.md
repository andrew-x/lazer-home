...
- **"Add company" / "Add contact" dialogs** (`add-company-dialog.tsx` / `add-contact-dialog.tsx`) render **only when the user holds the matching create permission** — the page checks `userHasPermission(user, { companies: ["create"] })` / `{ contacts: ["create"] }` and conditionally renders the trigger. The server action is gated regardless, so this is an affordance, not the boundary. Forms use the tight `useHookFormAction` binding (see `.claude/rules/forms.md`); the contact form's company picker is `company-combobox.tsx`.
- **Create a company inline from the contact form.** `add-contact-dialog.tsx` puts a **"New" button next to the company picker** that reuses `AddCompanyDialog` (a **stacked dialog**) via its optional `trigger` + `onCreated` props. On create, `onCreated` reports `{ id, name }` and the contact form auto-selects the new company in its `company-combobox` — no refetch round-trip. (Website is an optional field; the add-company dialog has no partner subtitle.)
- **`company-combobox.tsx`** — a debounced, searchable company picker built on the vendored `combobox` (Base UI Combobox). Built-in filtering is disabled (`filter={null}`) — filtering is server-side: it runs `searchCompanies` via `useAction`, keyed off the input debounced through `useDebouncedValue` (`src/hooks/`, 250ms), searches on mount with an empty query (first results), and is clearable (a contact's company is optional). The selected company is kept in the item list even when absent from current results, so its label/selected state render. Selecting reports `{ id, name }`. See [domains/crm.md](./domains/crm.md).
