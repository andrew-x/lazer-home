...
**Status: built.** The authenticated UI shell, auth screens, and error/404/loading conventions exist, plus real domain pages: staff (directory + profiles), CRM (`/companies`, `/contacts`, `/opportunities`), and projects (`/projects`). The home dashboard is still placeholder stat cards. Path-scoped working rules are in `.claude/rules/ui.md` (loads when you touch `src/components/**` or `src/app/**`) — read it before writing UI code; this doc is the architectural *why*.
...
Pages today: `/login` (landing + Google button), `/` (home dashboard with placeholder stat cards), `/staff` ("Browse staff" — searchable/filterable directory of everyone) and `/staff/[id]` (a person's profile), `/profile` ("My profile" — your own staff profile, same presentation as `/staff/[id]`), `/companies` ("Companies" — the CRM company list, see below), `/contacts` ("Contacts" — the CRM people list, see below), `/opportunities` (the CRM pipeline, see below), `/projects` (client engagements + staffing, see below), `/settings` (account info synced from Google + sign out), `/profile-setup` (the single onboarding block screen, two messages), plus the localhost-only `/admin` surface (see below). `NAV_ITEMS` (`src/components/app-shell/nav.ts`) has **eight entries**: Home (`IconHome`), My profile → `/profile` (`IconUser`), Staff (`IconUsers`), Companies → `/companies` (`IconBuildingSkyscraper`), Contacts → `/contacts` (`IconAddressBook`), Opportunities → `/opportunities` (`IconTargetArrow`), Projects → `/projects` (`IconBriefcase`), Settings (`IconSettings`). Admin is **not** in the nav (localhost-only tooling, reached by URL).
...
- **`companies-table.tsx` / `contacts-table.tsx`** are presentational (plain shadcn `table`). The companies table shows a **"Partner" badge only when `isPartner` is true** — there is no "Client" badge (a company is neither client nor partner by default; see [domains/crm.md](./domains/crm.md)). The contacts table shows the joined `companyName`, the resolved `managerName` (the "Manager" column, `—` when none), and `linkedinUrl` as an external "Profile" link (`IconBrandLinkedin`, opens in a new tab) in a "LinkedIn" column.
...
- **Manager picker — `manager-combobox-field.tsx` (`ManagerComboboxField`).** A searchable, debounced single-select of *other contacts at the same company*, mirroring `CompanyComboboxField`'s `{ id, name } | null` shape. It passes the form's chosen `companyId` to `searchContacts` (which scopes results to that company), and is **disabled until a company is chosen** (a contact with no company can't have a manager). Clearable, since a manager is optional. The same-company rule is also re-enforced server-side in `createContact` — see [domains/crm.md](./domains/crm.md), [ADR 0020](./decisions/0020-contact-manager-self-reference.md).
...
- **`entity-combobox.tsx`** (`src/components/crm/`) — the **single-select** sibling of `EntityMultiCombobox`, added for a role's one staff member. Server-filtered via an injected search action; reports `{ id, name } | null`. Search-only (no inline create).
