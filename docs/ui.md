...
- **"Add company" / "Add contact" dialogs** (`add-company-dialog.tsx` / `add-contact-dialog.tsx`) render **only when the user holds the `contacts.edit` capability** — the page computes one `canEdit = userHasPermission(user, { contacts: ["edit"] })` flag (a single gate covers both companies and contacts) and conditionally renders each trigger. The server actions are gated regardless, so this is an affordance, not the boundary. Forms use the tight `useHookFormAction` binding (see `.claude/rules/forms.md`); the contact form's company picker is `company-combobox.tsx`.
