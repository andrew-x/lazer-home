...
3. **Read — three tiers, enforced by projection.** Who sees what is the crux:
   ...
   - **Reviewer, browsing** — the same capability also backs the **"Your reports" tab** on `/feedback` (`getFeedbackAboutReports`): full-content rows whose *recipient* is one of the caller's **direct reports** (`staff.managerId` = caller, one hop, self-excluded), newest first. It lists nothing the caller couldn't already open at `/feedback/[id]`, so it's a browse surface over existing authorization; `null` (not permitted / no linked staff row) hides the tab entirely. There is still **no browse-all-feedback list** ([ADR 0047](./decisions/0047-feedback-reports-scoping-not-granting.md)).
...
> **Authorization here is still purely role-based:** "Manager" as a *gate* means the `feedback.review` **capability**, and no permission check consults the reporting line. What changed is *scoping* — a per-person reporting graph **does** exist (`staff.managerId`, [ADR 0026](./decisions/0026-staff-manager-self-reference.md)) and the "Your reports" tab reads it to **narrow** an already-permitted result set. Scoping ≠ granting; earlier text here claiming "there is no manager/report graph in the system" was stale.
