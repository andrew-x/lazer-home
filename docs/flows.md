...
> **Editing a company / contact** uses the same gated pattern as create, but the affordance lives on the **detail page** (`/companies/[id]`, `/contacts/[id]`): the page computes `canEdit = userHasPermission(user, { crm: ["edit"] })` and renders the Edit dialog only when set, while `updateCompany` / `updateContact` re-declare `metadata({ permission: { crm: ["edit"] } })` as the real boundary. Both edit an optional **staff owner** via `OwnerComboboxField` (backed by the `crm.edit`-gated `searchStaff`); `updateContact` re-runs the same-company manager check and email-uniqueness map that `createContact` uses. Each revalidates its list path *and* its detail path. No CRM delete exists yet.
