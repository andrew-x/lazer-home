...
> **Projects** (`/projects`) follow the same gated-create pattern, gated on **`projects.edit`** instead of `crm.edit`: the page hides `AddProjectDialog` unless `userHasPermission(user, { projects: ["edit"] })`, and `createProject` declares `metadata({ permission: { projects: ["edit"] } })`. Its standalone create form is **deliberately minimal — only name + company** (a project has **no stored status or line of business**; both are *derived* from its roles — [ADR 0033](./decisions/0033-line-of-business-on-role-derived-project-status.md)); **roles and delivery managers are added afterward in the planner** (`createProject`'s `roles`/`deliveryManagerIds` default to empty; each role carries its own line of business). Creating a project **from an opportunity** skips this dialog entirely — it's a one-click `createProjectFromOpportunity` (inherits the opportunity's name + company, no roles). Editing a project's name/delivery managers happens later via `updateProject` (also `projects.edit`-gated) from the planner's Edit-project dialog; roles have their own per-role planner actions. Its staff/company pickers use `projects.edit`-gated search actions sharing the query bodies in `src/actions/shared/entitySearch.ts`. See [domains/projects.md](./domains/projects.md).
