...
5. **Review (Performance).** A Person's project work and utilization inform assessment and growth. _(Two slices built: continuous **peer feedback** (staff → staff — see the peer-feedback flow below) and a **compensation & headcount analytics dashboard** (`/performance`, `staff.viewCompensation`-gated, CAD/USD toggle over live FX rates — see [domains/performance.md](./domains/performance.md), [ADR 0029](./decisions/0029-external-fx-rates-and-currency-normalization.md)). The formal machinery — ReviewCycle, PerformanceReview, Goals — is still proposed.)_
...
> **Projects** (`/projects`) follow the same gated-create pattern, gated on **`projects.edit`** instead of `crm.edit`: the page hides `AddProjectDialog` unless `userHasPermission(user, { projects: ["edit"] })`, and `createProject` declares `metadata({ permission: { projects: ["edit"] } })`. Its form has a project-level **line of business** select plus a `useFieldArray` roles repeater requiring **at least one** role (each row: role type + optional staff (blank ⇒ placeholder) + optional name + dates + hours — no per-role line of business, [ADR 0025](./decisions/0025-line-of-business-on-opportunity-and-project-not-role.md)); `createProject` writes the project (with its `lineOfBusiness`) + delivery-manager junction rows + role rows in one transaction, and sets the optional `opportunityId`. Its staff/company pickers use `projects.edit`-gated search actions sharing the query bodies in `src/actions/shared/entitySearch.ts`. See [domains/projects.md](./domains/projects.md).
...
> **Editing an opportunity** uses the same middleware but is an *update*: clicking a board card opens the two-tab detail drawer, which lazy-loads via `loadOpportunityDetail` (a `crm.edit`-gated interactive-read action wrapping the server-only `getOpportunity`). Its **Info** tab edits **one field at a time in place** — each Confirm calls `updateOpportunity` (updates the row — including the now-editable company — + replaces the four people junctions wholesale) with the full input rebuilt from the loaded detail plus that one changed field, then re-fetches. The **Project plan** tab holds the single linked project (create hidden once one exists).
...
3. **Enforce (server).** Both `updateOpportunityPosition` and `updateOpportunity` fetch the current status and re-check the guard **transition-based** — `newStatus !== currentStatus && requiresProject(newStatus) && !opportunityHasProject(id)` — throwing a `UserSafeActionError` only when the move actually enters a delivery stage without a project. The client block is only UX.
4. **Link.** `createProject` sets `projects.opportunityId` and revalidates `/projects` *and* `/opportunities` (so the board's `hasProject` refreshes). An opportunity has **at most one** project — `createProject` rejects a second one (`opportunityHasProject` → user-safe error, backing the partial unique index; [ADR 0019](./decisions/0019-project-opportunity-link.md)), and the drawer hides "Create project" once linked. Projects created at the Allocating stage typically carry **placeholder roles** (staff not yet chosen).
