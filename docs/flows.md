...
> **Editing an opportunity** uses the same middleware but is an *update*: clicking a board card opens the detail drawer, which lazy-loads via `getOpportunityDetail` (a `crm.edit`-gated interactive-read action wrapping the server-only `getOpportunity`) and saves via `updateOpportunity` (updates the row + replaces the four people junctions wholesale; company not editable).
...
4. **Link.** `createProject` sets `projects.opportunityId` and revalidates `/projects` *and* `/opportunities` (so the board's `hasProject` refreshes). Projects created at the Allocating stage typically carry **placeholder roles** (staff not yet chosen).
