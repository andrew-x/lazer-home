...
The seam between CRM and delivery, now built ([ADR 0024](./decisions/0024-opportunity-project-handoff-and-placeholder-roles.md)). An opportunity can't advance into a **delivery stage** (the Allocating group onward, `requiresProject(status)`, Closed – Lost excepted) without a linked project.

1. **Trigger.** A `crm.edit` user drags a card into Allocating+ (board) — or edits its status to one in the drawer. The board computes the destination `{ status, position }`.
2. **Block + branch (client).** If `requiresProject(newStatus)` and the card has no project (`hasProject`, from `getOpportunitiesBoard`), the move is **not applied**. Then: a user with **`projects.edit`** (`canCreateProject`) gets `AddProjectDialog` **prefilled + company-locked** for that opportunity; on `onCreated`, the pending status move completes (`updateOpportunityPosition`). A user **without** `projects.edit` (e.g. `sales`) gets a toast — a delivery manager must create the project first. No RBAC was widened.
3. **Enforce (server).** Both `updateOpportunityPosition` and `updateOpportunity` re-check `requiresProject` + `opportunityHasProject` and throw a `UserSafeActionError` if the delivery-stage move has no project — the client block is only UX.
