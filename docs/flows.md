...
> **Editing an opportunity** uses the same middleware but is an *update*: clicking a board card opens the two-tab detail drawer, which lazy-loads via `loadOpportunityDetail` (a `crm.edit`-gated interactive-read action wrapping the server-only `getOpportunity`). Its **Info** tab edits **one field at a time in place** — each Confirm calls `updateOpportunity` (updates the row + replaces the four people junctions wholesale; company not editable) with the full input rebuilt from the loaded detail plus that one changed field, then re-fetches. The **Project plan** tab holds the single linked project (create hidden once one exists).
