...
**`db` is imported only from `src/actions/**`.** Pages and components (even SSR Server Components) go through the actions layer, never query Drizzle directly — see `.claude/rules/server-actions.md` ("The actions layer owns all DB access").
