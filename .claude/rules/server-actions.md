...
- **Revalidate** after mutations: `revalidatePath(...)` / `revalidateTag(...)` from `next/cache`. **Exception — autosave endpoints:** an action fired on every debounced keystroke must not revalidate the route the person is typing on, or the server re-renders the editor out from under them. Revalidate only *other* routes that read the data (`upsertResponse` revalidates the profile pages, not the survey editor), or nothing at all when the editing surface is the only reader (`saveCompensationPlanItem`) — the client owns its own state and navigation re-reads.
...
Each `*.schema.ts` also exports its inferred input type — `export type XInput = z.infer<typeof xSchema>` — as the canonical per-action input type. Keep this export even when it is currently consumed only in-file: it is the intended public contract for the action's input, not accidental dead code.
