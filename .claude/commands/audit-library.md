...
argument-hint: "[optional: a doc or area to scope the audit, e.g. \"crm\" or \"data-model\"]"
---
...
Scope for this run: **$ARGUMENTS** (if empty, audit every doc under `/docs`).
...
Main context is gold, and the librarian owns `/docs`. **Dispatch the `librarian`
subagent (via the Agent tool) in read-only audit mode** — do not read the whole tree
yourself. For coverage and speed, run **one librarian per doc (or doc group)** in
parallel, each scoped to a single doc and told to:
...
If `$ARGUMENTS` narrows the scope, only dispatch librarians for the matching docs.
Then synthesize their conclusions into the single report below.
