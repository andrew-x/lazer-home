...
argument-hint: "[optional: a domain or path to scope the audit, e.g. \"crm\" or \"src/actions/crm\"]"
---
...
Scope for this run: **$ARGUMENTS** (if empty, audit the whole `src/` tree).
...
- **`.claude/rules/`** — the path-scoped working rules: `server-actions.md`,
  `database.md`, `forms.md`, `ui.md`, `nextjs.md`. These are the concrete "how we
  write X here" contracts. A deviation from a rule is a finding.
...
Main context is gold. Don't read the whole tree yourself. **Dispatch the dimensions
below as parallel subagents** (Explore / general-purpose via the Agent tool), each
scoped to one dimension and instructed to return only a structured list of findings
(`file:line`, what, why it matters, suggested fix, severity) — not file dumps. Then
synthesize their conclusions into one report. If `$ARGUMENTS` narrows the scope,
pass that scope to every agent.
...
line — and ask me to choose before anything gets applied. Prefer surfacing these as an
explicit multiple-choice decision (via `AskUserQuestion` when running interactively),
with your recommended option first and marked as such. Note which findings are blocked
on my decision so the punch list stays actionable. Reserve this for real forks — if
one approach is clearly correct for our conventions, just recommend it and cite the
rule or ADR.
...
findings by what they touch, and dispatch one subagent per independent group (via the
Agent tool), each with a tight brief: the specific findings, the chosen approach, and
the relevant `.claude/rules/` contract. Fixes that touch the same files, or that
depend on another fix landing first (e.g. extract a helper before its call sites
